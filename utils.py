...
from model import *
from datasets import FEMNIST,CIFAR10_truncated, CIFAR100_truncated, ImageFolder_custom
...
def load_femnist_data(datadir):
    ...
    mnist_train_ds = FEMNIST(datadir, train=True, download=True, transform=transform)
    mnist_test_ds = FEMNIST(datadir, train=False, download=True, transform=transform)
    ...
    #u_train=u_train.data.numpy()
    #u_test=u_test.data.numpy()
    return (X_train, y_train, X_test, y_test,u_train,u_test)
...
def partition_data(dataset, datadir, logdir, partition, n_parties, beta=0.4):
    if dataset == 'cifar10':
        ...
        X_train, y_train, X_test, y_test,u_train,u_test = load_femnist_data(datadir)
...
def get_dataloader(dataset, datadir, train_bs, test_bs, dataidxs=None, noise_level=0):
    if dataset in ('cifar10', 'cifar100'):
        ...
        train_dl = data.DataLoader(dataset=train_ds, batch_size=train_bs, shuffle=True, drop_last=False,num_workers=8)
        test_dl = data.DataLoader(dataset=test_ds, batch_size=test_bs, shuffle=False, drop_last=False,num_workers=8)
    elif dataset == 'tinyimagenet':
